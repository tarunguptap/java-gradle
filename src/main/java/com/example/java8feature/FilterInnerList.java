...
public class FilterInnerList {
	...
	private static List<Customer> filetrOuterListBasedOnInnerList() {
        ...
        for (Customer customer3 : customerList) {
        	System.out.println("Before Items count >>>>>>>>> " + customer3.getObjectName() + " >>>>>>> " + CollectionUtils.size(customer3.getOrders()));
		}
		List<Customer> newCustomerList = getCustomerList().stream()
        .filter(cust -> CollectionUtils.size(cust.getOrders().stream().filter(itm -> Objects.isNull(itm.getOrderCompletionDate())).collect(Collectors.toList()))>0)
        ...
        //Console Output :
        /*Before Customer Count>>>>>>>>> 3
		Before Items count >>>>>>>>> customer >>>>>>> 1
		Before Items count >>>>>>>>> customer1 >>>>>>> 3
		Before Items count >>>>>>>>> customer2 >>>>>>> 1
		After >>>>>>>>> 2
		Before Items count >>>>>>>>> customer1 >>>>>>> 3
		Before Items count >>>>>>>>> customer2 >>>>>>> 1*/
	}
	...
	private  static List<Customer> filetrInnerListElements() {
        ...
        List<Customer> newCustomerList = customerList.stream()
            .map(cust -> {
            	List<Order> orders = cust.getOrders().stream().filter(ordr -> Objects.isNull(ordr.getOrderCompletionDate())).collect(Collectors.toList());
            	cust.setOrders(orders);
                return cust;
            })
            .filter(cust -> CollectionUtils.size(cust.getOrders()) > 0)
            .collect(Collectors.toList());
       ...
       /*Before Customer Count>>>>>>>>> 3
		...
		Before Items count >>>>>>>>> customer2 >>>>>>> 1
		After >>>>>>>>> 2
		Before Items count >>>>>>>>> customer1 >>>>>>> 1
		Before Items count >>>>>>>>> customer2 >>>>>>> 1*/
	}
